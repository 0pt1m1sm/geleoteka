...
|---|---|---|---|
| **X1** | Почта: уйти с Resend на провайдеро-независимую отправку | **ЗАБЛОКИРОВАНО ПЛАТФОРМОЙ** | Проверено из самого приложения 01.09: `smtp.timeweb.ru` не отвечает ни на одном почтовом порту — 465, 587, 2525 и 25 дают таймаут, то есть Timeweb режет исходящий SMTP с App Platform. С рабочей машины тот же сервер авторизует нормально, поэтому дело не в доступах и не в наших правилах файрвола. Отправка возвращена на Resend. Дальше два пути, и выбор за владельцем: попросить поддержку Timeweb открыть исходящий SMTP, либо завести второй HTTP-адаптер (HTTPS проходит) — тогда провайдеро-независимость достигается двумя API-отправителями, а не SMTP. Проверка теперь машинная: `POST /api/internal/email/self-test` |
...
1. **X1** — выбрать путь: просить Timeweb открыть исходящий SMTP или заводить второй HTTP-адаптер отправки взамен Resend.
