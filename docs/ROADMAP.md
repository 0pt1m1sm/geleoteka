...
- **Осталось:** пишущие пути на шов (там ошибка портит данные, а не показывает меньше —
  делается при владельце), RLS и отдельная роль БД (7, проект и замеры готовы), разделение
  идентичности (9), per-tenant уникальности (10, разбор готов), подставной второй тенант (11).
