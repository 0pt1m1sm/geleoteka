...
| **O2** | Склад: расхождения при приёмке и возвраты | **НЕТ СПЕКИ** | найдено аудитом 2026-07-11 | принял меньше, чем в накладной — расхождение зафиксировано, а не подогнано |
| **O3** | Сбор дефектов с живых устройств | **НЕТ СПЕКИ** (постановка есть) | Story 8 в `2026-07-31-platform-foundations.md` | ошибки JS, упавшие запросы и переполнение вёрстки сами складываются в кучу; без значений полей и токенов |
...
|---|---|---|---|
| **X1** | Почта: перевести отправку на SMTP, убрать Resend | я | на проде `EMAIL_TRANSPORT=resend`, при этом SMTP-доступы уже заданы. Одно изменение конфига + смоук. Провайдеро-независимость — требование платформы |
...
| **X8** | Срок жизни claim-токена | я | TD-002: гашение при клейме сделано, срока жизни нет — невостребованный токен живёт вечно |
