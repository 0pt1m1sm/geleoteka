...
|---|---|---|---|
| **X1** | Почта: остаёмся на Resend | **РЕШЕНО 01.09** | Уйти на SMTP на этом хостинге нельзя: Timeweb режет исходящий SMTP как класс: из приложения в таймаут уходят и 465/587/2525/25 до `smtp.timeweb.ru`, и порты Gmail, Яндекса и Mail.ru — то есть дело не в маршруте до их почты. С рабочей машины те же серверы отвечают, а наш файрвол разрешает исходящий TCP на любой порт. **Решение владельца: пока остаёмся на Resend.** Обращение в поддержку Timeweb с просьбой открыть 465/587 подготовлено — создать тикет через API нельзя, он только на чтение. Если поддержка откажет, провайдеро-независимость даёт второй HTTP-адаптер отправки, а не SMTP. Состояние отправки проверяется машинно: `POST /api/internal/email/self-test` |
...
1. **X1** — отправить подготовленное обращение в поддержку Timeweb (создать тикет через API нельзя). До ответа остаёмся на Resend по решению владельца.
