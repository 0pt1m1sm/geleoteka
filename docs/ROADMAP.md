...
### P4. Клиентское приложение — PWA под брендом сервиса — **ГОТОВО К СТАРТУ**

- **Спека:** `docs/plans/2026-09-01-mobile-client-app.md` · 7 историй
- **Целевое состояние:** у клиента на экране телефона иконка ЕГО сервиса; пуш о готовой смете
  приходит туда же. Решение 02.09: нативного приложения под брендом платформы не будет —
  человек приехал в Гелеотеку и приложение с незнакомым именем не поставит.
- **Следствие:** отдельный API клиентской части не нужен, PWA переиспользует уже написанный
  кабинет. P2 сокращается до нужд админского приложения.
