...
- **Сделано в ночь на 02.09** (всё выкачено и проверено на проде):
  ...
  6. Контекст запроса и `tenantDb()`; переведено 7 модулей из ~200, откат назад закрыт
     храповиком.
  8. Долг проверок по роли зафиксирован числом (209) — расти больше не может.
- **Осталось:** массовый перевод чтений (Story 6), RLS и отдельная роль БД (7), разделение
  идентичности (9), per-tenant уникальности (10), подставной второй тенант (11).
