...
|---|---|---|---|
| **X1** | Почта: отправка на SMTP вместо Resend | сделано, ждёт подтверждения | **Переключено 01.09** с разрешения владельца: `EMAIL_TRANSPORT=smtp`, приложение выкачено. Проверено: строки `EMAIL_TRANSPORT` в таблице настроек нет, значит правит переменная окружения; правила файрвола разрешают исходящий TCP на любой порт; `smtp.timeweb.ru:465` авторизует, видимый `From` равен ящику отправки. **Не доказано:** пускает ли сеть App Platform исходящий 465 — это видно только реальной отправкой. Кнопка «Отправить тестовое письмо» в `/admin/settings/integrations` покажет точный ответ транспорта. Риск ограничен: отправка идёт fire-and-forget, отказ не ломает страницы, а исходящих писем в журнале за всё время одно. Откат — одно изменение обратно |
...
1. **X1** — нажать «Отправить тестовое письмо» в `/admin/settings/integrations`: это единственная проверка, доказывающая, что сеть платформы пускает исходящий SMTP.
