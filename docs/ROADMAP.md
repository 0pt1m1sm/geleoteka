...
|---|---|---|---|
| **X1** | Почта: остаёмся на Resend | **РЕШЕНО 01.09** | Уйти на SMTP на этом хостинге нельзя: Timeweb режет исходящий SMTP — 465, 587, 2525 и 25 из приложения дают таймаут, при том что с рабочей машины тот же сервер авторизует, а наш файрвол разрешает исходящий TCP на любой порт. **Решение владельца: пока остаёмся на Resend.** Обращение в поддержку Timeweb с просьбой открыть 465/587 подготовлено — создать тикет через API нельзя, он только на чтение. Если поддержка откажет, провайдеро-независимость даёт второй HTTP-адаптер отправки, а не SMTP. Состояние отправки проверяется машинно: `POST /api/internal/email/self-test` |
