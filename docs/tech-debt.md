...
**Status:** ЗАКРЫТ 2026-09-01. Гашение при клейме было сделано раньше
(`app/actions/customer-onboarding.ts` обнуляет `claimToken` на всех трёх путях); срок жизни
добавлен 01.09 — `CLAIM_TOKEN_TTL_MS` = 14 дней от оформления заказа, проверка на обеих
дверях (создание пароля и вход с привязкой заказа), тесты `tests/auth/claim-token-ttl.test.ts`.
Отдельная колонка не понадобилась: токен выдаётся только при создании заказа, поэтому его
возраст равен возрасту заказа. Согласование сметы гостем осознанно не ограничено сроком —
