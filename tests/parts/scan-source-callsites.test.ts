...
const CALLSITE_INVARIANTS: ReadonlyArray<{
  ...
  },
  {
    ...
    must: /condition:\s*"NEW"/,
    why:
      "карта перечисляет ещё и товары БЕЗ номенклатуры, а страница закрывает " +
      "от индексации всё, что не новое: без фильтра карта заявляет адреса, " +
      "которые сами себя закрывают. Прошлое обоснование снятия («карта товаров " +
      "не касается») было неверным — второй запрос касается",
  },
