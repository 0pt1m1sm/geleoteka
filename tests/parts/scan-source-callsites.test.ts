...
const CALLSITE_INVARIANTS: ReadonlyArray<{
  ...
  // СНЯТО в Story 6 осознанно, вместе с самим предметом инварианта. Он требовал
  // от карты сайта фильтра по состоянию товара, потому что раньше она
  // перечисляла адреса ТОВАРОВ. После решения Р1 канонический адрес детали —
  // страница по номеру, и карта перечисляет номенклатуру: товаров она вообще
  // не касается, фильтровать их по состоянию там больше негде и незачем.
  // Замена инварианта — не подстрочная: тесты карты сайта проверяют, что в неё
  // попадают номенклатуры с живым товаром и не попадают пустые.
  // СНЯТО в Story 3 осознанно: витрина теперь ПОКАЗЫВАЕТ б/у, потому что у
  // детали появилась карточка с вариантами, метка состояния в списке и
  // canonical на хозяина. Инвариант убран вместе с заглушкой — и тест
  // потребовал сделать это явно, а не молча, ради чего он и написан.
  // СНЯТ: этот инвариант проверял лишь наличие подстроки `pickVariantHost(` в
  // файле и был ЗЕЛЁНЫМ при живом блокере — исполнение до вызова не доходило,
  // потому что выше стоял ранний выход. Его заменил ИСПОЛНЯЕМЫЙ
  // tests/parts/part-page-metadata.test.ts, а слабый сторож удалён, чтобы не
  // создавать ложного чувства покрытия.
];
