...
function wire() {
  ...
  const projectInboundEvents = async (): Promise<void> => {
    projectionCalls += 1;
  };
  ...
  const deps: SyncDeps = {
    ...
    port,
    mapper,
    ingest: (parsed) => ingestEmail(parsed, { client: db, projectInboundEvents }),
    sleep: async () => {},
  };
  ...
  return { db, port, deps, config, projectionCalls: () => projectionCalls };
}
...
describe("replayDeadLetter — idempotent recovery", () => {
  it("replaying a fixed DEAD message creates exactly one CRM row and one task", async () => {
    const { db, port, deps, config, projectionCalls } = wire();
