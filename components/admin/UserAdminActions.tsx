  ...
  async function handlePurge(): Promise<void> {
    ...
    try {
      ...
      if (check.blockers.length > 0) {
        ...
        setError(
          `Здесь удаляются только пустые записи, а у этого пользователя есть данные (${detail}). Заблокируйте его, либо удалите вместе со всеми данными в блоке «Опасная зона» ниже — там сначала выгружается копия.`,
        );
        return;
      }
