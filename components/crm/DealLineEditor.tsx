...
function draftFrom(line: DealLineView): Draft {
  return {
    type: line.type,
...
/**
 * Polymorphic DealLine editor — replaces JobLineEditor's role on the
 * estimate side. Each line picks its `type` (LABOR / PART / RENTAL_DAY /
 * DISCOUNT / FEE) so a single editor handles every deal channel.
 */
export function DealLineEditor({
  dealId,
  initialLines,
  editable,
    ...
    <div className="space-y-4">
      {initialLines.length === 0 ? (
        <p className="text-sm text-[var(--foreground-muted)]">
          {editable ? "Строки ещё не добавлены." : "Сделка пуста."}
        </p>
      ) : (
...
function EditRow({
  ...
  const [state, formAction, isPending] = useActionState(updateDealLine, null);
  ...
  function handleDelete(): void {
    ...
    startDelete(async () => {
      await deleteDealLine(line.id);
    });
  }

  return (
    <form action={formAction}>
      <input type="hidden" name="dealLineId" value={line.id} />
      <RowFields
        index={index}
        draft={draft}
        rowTotal={total}
...
function AddRow({
  ...
  const [state, formAction, isPending] = useActionState(addDealLine, null);
