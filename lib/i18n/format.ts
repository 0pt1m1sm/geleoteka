...
/**
 * Настройки, по которым продукт работал до появления полей у арендатора.
 *
 * Нужны в двух местах: когда миграция ещё не накатилась и когда компонент
 * отрисован вне разметки (тест, изолированный просмотр). Объявлены здесь
 * единожды, чтобы тот же литерал не появлялся в третий раз.
 */
