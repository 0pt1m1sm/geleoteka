 ...
 * all. The destination bay is selected by the same row-locking allocator as
 * public booking, while Slot(dateTime, bayId) remains the exact-collision
 * database backstop.
 *
...
export interface RescheduleTx extends ServiceBayAllocationTx {
  repairOrder: {
...
export async function applyReschedule(
  ...
  try {
    await client.$transaction(async (tx) => {
      const bayId = await chooseAvailableServiceBay(tx, next, {
        excludeRepairOrderId: repairOrderId,
      ...
      if (slot) {
        await tx.slot.update({ where: { repairOrderId }, data: { dateTime: next, bayId } });
      } else {
        await tx.slot.create({ data: { repairOrderId, dateTime: next, bayId } });
      }
      await tx.repairOrder.update({ where: { id: repairOrderId }, data: { dateTime: next } });
    });
  } catch (err) {
    if (err instanceof NoServiceBayAvailable) return { ok: false, reason: "conflict" };
