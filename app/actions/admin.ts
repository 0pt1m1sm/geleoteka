...
export async function updateRepairOrderDetails(
  ...
  // Время записи живёт здесь же, но меняется не как остальные поля: оно двигает
  // ещё и слот в календаре и может упереться в чужую бронь. Поэтому идёт первым
  // и через ту же applyReschedule — иначе отказ по занятости оставил бы часть
  // формы сохранённой, а часть нет.
  const dateTimeRaw = formData.get("dateTime");
  if (typeof dateTimeRaw === "string" && dateTimeRaw.trim() !== "") {
    const next = parseDatetimeLocalInput(dateTimeRaw);
    if (!next) return { error: "Некорректные дата и время записи" };
