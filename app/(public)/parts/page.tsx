...
interface Props {
  searchParams: Promise<{ q?: string; category?: string; oem?: string; inStock?: string }>;
}
...
export default async function PartsPage({ searchParams }: Props) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <PartsSearch categories={cats} currentQuery={q} currentCategory={categorySlug} oemOnly={oemOnly} inStockOnly={inStockOnly} />

      <div className="text-sm text-[var(--foreground-muted)] mb-4">
        Найдено: {parts.length}
      </div>
              ...
              >
                ...
                <div className="aspect-square bg-[var(--background-secondary)] rounded-lg mb-3 flex flex-col items-center justify-center overflow-hidden relative">
                  {(part.photos as string[])?.length > 0 ? (
                    <img src={(part.photos as string[])[0]} alt={part.name as string} className="w-full h-full object-cover" />
                  ) : (
                    <>
                      <span className="text-4xl font-black text-[var(--color-accent)] opacity-15">G</span>
                      <span className="text-[10px] font-mono text-[var(--foreground-muted)] opacity-40 mt-1">{part.article as string}</span>
                    </>
                  )}
                </div>
                <div className="flex items-center gap-2 mb-2">
                  <span className={`badge text-[10px] ${(part.isOEM as boolean) ? "bg-[var(--color-accent)]/10 text-[var(--color-accent)]" : "badge-silver"}`}>
                    {(part.isOEM as boolean) ? "OEM" : "Аналог"}
                  </span>
                  {cat && <span className="text-[10px] text-[var(--foreground-muted)]">{cat.name}</span>}
                </div>
                ...
                <div className="flex items-center justify-between mt-auto pt-2 border-t border-[var(--border)]">
                  ...
                  <span className={`text-xs ${(part.quantity as number) > 0 ? "text-[var(--color-success)]" : "text-[var(--foreground-muted)]"}`}>
