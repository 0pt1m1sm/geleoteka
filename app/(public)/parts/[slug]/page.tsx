...
export default async function PartDetailPage({ params, searchParams }: Props) {
  ...
  if (refOem) {
    ...
    if (!isNew) {
      // Р2: у б/у и восстановленной СВОЕЙ страницы нет и не будет — адрес
      // переехал навсегда, и это можно заявлять постоянным редиректом, потому
      // что цель больше не переезжает. Параметр клеим, только пока экземпляр
      // жив: у проданного он развернул бы на странице карточку, которой нет,
      // а сама страница и так скажет, что в наличии.
      permanentRedirect(
        p.isActive ? `${oemPath}?v=${encodeURIComponent(p.sku as string)}` : oemPath,
      );
    }
