...
export default async function CustomerDetailPage({ params }: Props) {
  ...
  const lifetimeValue = customer.customerProfile?.lifetimeValue ?? 0;
    ...
    <div>
      ...
      <CustomerTabs
        tabs={[
            ...
            content: (
              <div className="space-y-6">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <div className="card">
                    <p className="text-sm text-[var(--foreground-muted)]">Lifetime value</p>
                    <p className="text-2xl font-bold tabular-nums text-[var(--color-accent)]">{formatPrice(lifetimeValue)}</p>
                  </div>
                  <div className="card">
                    <p className="text-sm text-[var(--foreground-muted)]">Последний контакт</p>
                    <p className="text-lg font-semibold">{lastTouchAt ? formatDate(lastTouchAt) : "—"}</p>
                  </div>
                  <div className="card">
                    <p className="text-sm text-[var(--foreground-muted)]">Первое касание</p>
                    <p className="text-lg font-semibold">{firstSeenAt ? formatDate(firstSeenAt) : "—"}</p>
                  </div>
                  <div className="card">
                    <p className="text-sm text-[var(--foreground-muted)]">Источник</p>
          ...
          {
            key: "vehicles",
            label: `Автомобили (${customer.vehicles.length})`,
            content: (
