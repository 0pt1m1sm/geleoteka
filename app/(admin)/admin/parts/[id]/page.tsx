...
export default async function EditPartPage({ params }: Props) {
  ...
  const serialized = {
    ...
    quantity: (p.stockItems as Array<{ quantity: number }>)[0]?.quantity ?? 0,
    barcode: (p.stockItems as Array<{ barcode: string | null }>)[0]?.barcode ?? "",
    gtin: (p.stockItems as Array<{ gtin: string | null }>)[0]?.gtin ?? "",
